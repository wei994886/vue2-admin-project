<template>
  <div class="tabs_box">
    <div class="tabs_content">
      <el-tabs :closable="tabList.length > 1" v-model="active" @contextmenu.prevent.native="openContextMenu($event)"
        type="editable-card" @tab-remove="removeTab" @tab-click="tabClick">
        <el-tab-pane class="isActive" v-for="item in tabList" :key="item.name" :label="item.name" :name="item.path">
        </el-tab-pane>
      </el-tabs>
    </div>
    <!-- 右键菜单 -->
    <ul v-show="contextMenu.isShow" :style="{ left: contextMenu.menuLeft + 'px', top: '100px' }"
      class="el-dropdown-menu el-popper" x-placement="bottom-end">
      ...
      <li v-if="this.active == this.contextMenu.name" class="el-dropdown-menu__item" @click="refresh">
...
export default {
  ...
  methods: {
    ...
    // 判断当前页
    getThisPage() {
      let currentPgae = this.$route, index = this.tabList.findIndex(tag => tag.path == currentPgae.path);
    ...
    // 关闭右侧
    closeRightTag() {
      let currentPath = this.contextMenu.name, index = this.tabList.findIndex(tag => tag.path == currentPath);
    ...
    // 关闭其它
    closeOtherTag() {
      let currentPath = this.contextMenu.name, currentName = this.contextMenu.label;
      this.tabList = [
        {
          name: currentName,
          path: currentPath
